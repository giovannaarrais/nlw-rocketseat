//bibliotecas(essa foi a dayjs) e codigos de terceiros
const formatador = (data) => {
    return {
        dia: {
            numerico: dayjs(data).format('DD'),
            semana: {
                curto: dayjs(data).format('ddd'), //time.short
                longo: dayjs(data).format('dddd'), //time.full
            }
        },
...
const atividade = {
    nome: "Almoço",
    data: new Date("2024-07-08 10:00"), //new date = criar data tem q ser nesse formato 
    finalizada: true
}
...
//logica para se true marcar o checkbox
   //se aql atividade for encontrada vai marcar o check
    let input = `
    <input 
...
const atualizarListaDeAtividades = () => {
    //query selector = pesquisa pelo seletor, atribui ele ao section do html
    ...
    for(let atividade of atividades) {//para cada atividade de atividades
    section.innerHTML += criarItemDeAtividade(atividade)
    //+= vai somar ao anterior e atribuir ao novo valor
   }
}
//innerHTML > vai substituir o html pelo q esta aqui
...
const salvarAtividade = (event) => {
    event.preventDefault()//nao faça oq é padrao
...
//vai buscar se tem atividade na mesma data
...
//se nao vai continuar atualizando
...
const criarDiasSelecao = () => {
    ...
    let diasSelecao = ''
//para cada dia de dias
    for (let dia of dias) {
        const formatar = formatador(dia)
        const diaFormatado = `${formatar.dia.numerico} 
        de ${formatar.mes}`
...
//se nao for igual ! = nega algo
    if(!atividade) {
        return
    }

    atividade.finalizada = !atividade.finalizada //atraves do ! se era false se torna verdadeiro e ao contrário
}